...
if(isset($_POST['btn'])){
    ...
    $task=$_POST['task_id_fk'];
    $student=$_POST['student_id_fk'];
    ...
    if(isset($_FILES['files'])) {
        ...
        foreach ($_FILES['files']['name'] as $i => $name) {
            ...
            if (move_uploaded_file($_FILES['files']['tmp_name'][$i], $upload_dir . $filename)) {
                ...
                $sql = "INSERT INTO student_files (path, task_id_fk, student_id_fk) VALUES ('$filename', '$task', '$')";

                // Execute the query
                if (mysqli_query($con, $sql)) {
                    echo '<script>alert("File uploaded successfully: "); window.location.href="student_post.phpinser.php";</script>';
